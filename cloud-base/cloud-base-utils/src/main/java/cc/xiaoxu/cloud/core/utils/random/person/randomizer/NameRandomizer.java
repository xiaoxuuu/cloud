...
public class NameRandomizer {
    ...
    public static String get() {
        return get(NameControl.of(), ThreadLocalRandom.current());
    }
     ...
     * @return 获取的人名
     */
    public static String get(NameControl nameControl) {
        return get(nameControl, ThreadLocalRandom.current());
    }
     ...
     * @return 获取的人名
     */
    public static String get(NameControl nameControl, ThreadLocalRandom localRandom) {

        List<String> surnameList = new ArrayList<>();
        if (nameControl.isUseSurname()) {
            surnameList.addAll(List.of(SURNAME));
        }
        if (nameControl.isUseDoubleSurname()) {
            surnameList.addAll(List.of(DOUBLE_SURNAME));
        }

        List<String> nameList = new ArrayList<>();
        if (nameControl.isUseName()) {
            nameList.addAll(List.of(NAME));
        }
        if (nameControl.isUseDoubleName()) {
            nameList.addAll(List.of(DOUBLE_NAME));
        }

        int surnameLength = localRandom.nextInt(surnameList.size());
        String finalSurname = surnameList.get(surnameLength);

        int nameLength = localRandom.nextInt(nameList.size());
        String finalName = nameList.get(nameLength);

        if (nameControl.getDesensitizationLength() != 0) {
            int index = Math.min(nameControl.getDesensitizationLength(), finalName.length());
            String last = finalName.substring(index);
            String desensitizationRepeat = String.valueOf(nameControl.getDesensitizationName()).repeat(index);
